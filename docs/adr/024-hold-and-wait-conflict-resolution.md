...
internally — it never renews, never replays the body, and never surfaces the
...
merely lost a race never discards its executed body. Read-writer validation runs
...
its whole identity, releases its locks (its transaction record is marked
`aborted`), mints a fresh id with `TxId::renew`, and replays the body from
...
- the transaction **body is replayed** and **every shard is re-CAS'd**,
  discarding work already done;
- each `renew` **orphans an aborted transaction record**, which is the dominant
...
the MVP, the contention win (no body replay, no orphaned objects, less GC
...
by wound-wait priority, it no longer returns a conflict that aborts the transaction.
...
transaction record. When `L` finalizes it re-resolves the entry and proceeds:
...
### Locks and the transaction record persist across the wait
...
in their shards/root and its transaction record stays `pending` (materialized
lazily, below) throughout the wait, so the work already done (executed body,
acquired locks) is preserved. A fresh id (`TxId::renew`) and a body replay are
...
it neither renews the id nor replays the body: `Algo` releases and re-acquires in
...
and left the read's shard **unlocked** on a stale bail-out, so the body replay had
to re-acquire it.
...
compares it to the observed token, reusing the exact routine optimistic validation
...
### The pending record stays lazily created (no upfront prepare)

The MVP's lazy creation is **kept**: a transaction does **not** write a pending
record before locking, so a short transaction that commits quickly still writes
its transaction record exactly once (directly as `committed`) and pays no extra
round-trip on the fast path. The record is materialized only when a hold outlives
the refresh interval — written by the refresher (below) after `PENDING_TX_TIMEOUT/2`.

A held lock whose pending record does not exist yet is still safe, because the two
facts a peer needs are available without it:

- **Priority** is embedded in the txid itself, so wound-wait orders the holder and
  the contender with no transaction record to read.
- **Liveness** is supplied by the observer-relative grace period
  ...
  below): a peer that resolves a `locked_by` entry to a *missing* transaction record treats the
  holder as pending until the record fails to **appear within `PENDING_TX_TIMEOUT`
  of first sight**, so it neither reclaims it prematurely nor blocks forever. The
  refresher materializes the real pending record well inside that window (at
...
before the record exists, and the refresher covers the rest of the hold. The cost
is that the engine relies on `handle_unknown_tx` rather than a guaranteed-present
pending record — but that grace path already exists and is exercised by crash
...
- **Absolute lease (foreign clock — skew applies).** When `L`'s pending record
...
- **Observer-relative progress (one clock — no skew).** The waiter also remembers,
  ...
  "Progress" is the pending record's `timestamp` **changing** or, when the transaction record is
  *missing*, the record **appearing** — so a missing transaction record must materialize within
...
relative check for an existing transaction record that stops refreshing while a waiter watches
...
Creating (rather than blindly writing) the record is what keeps lazy
materialization **wound-safe**: if an older peer already wounded this transaction
by writing an `aborted` record before it materialized its own pending one, the
...
CAS that finds the record `aborted` is the same wound signal once it has
...
loop over the locking step): the transaction keeps its **same id**, replays **no
...
- Under contention a transaction no longer discards executed work and acquired
  ...
  replays, far fewer orphaned transaction records, and therefore **less GC
...
- The **lease refresher becomes load-bearing**: it lazily *creates* the pending
  record (create-if-absent, wound-safe) once a hold outlives `PENDING_TX_TIMEOUT/2`
  and refreshes it thereafter. The MVP's lazy creation is kept — there is **no
  upfront prepare**, so the fast path still writes the transaction record only once
  (at commit) — and a held lock is bridged by the `handle_unknown_tx` grace period
  until the record exists. This re-activates ADR-021's refresher without restoring
