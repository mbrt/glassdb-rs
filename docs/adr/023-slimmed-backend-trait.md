...
backend trait and opaque revision-conditional read remain authoritative.

> Naming note: the caches were later renamed for clarity — `Global` →
> `ObjectCache` (backend-revision-keyed) and `Local` → `ValueCache`
...
- `get_metadata` — a tag/revision read with no body.
...
`metageneration` in the opaque revision token.
...
*revision-conditional* GET would let a hot, unchanged shard revalidate without
re-transferring its body. The slimmed trait must keep a conditional read; it just
has to be keyed on the revision, not on a tag.
...
```
read                 (path)                         -> ReadReply        // contents + revision
read_if_modified     (path, expected: &Revision)     -> ReadReply        // or Precondition if unchanged
write                (path, value)                  -> Revision
write_if             (path, value, expected: &Revision) -> Revision
write_if_not_exists  (path, value)                  -> Revision
delete               (path)                          -> ()
list                 (dir_path)                      -> Vec<String>
```

Content CAS (`write_if` / `write_if_not_exists`) is the only coordination
primitive. The opaque `Revision` token and the `BackendError` enum (including
`Unavailable`, the in-doubt marker of [ADR-009](009-in-doubt-conditional-writes.md))
are unchanged.

### Revision-conditional `read_if_modified` (implements the cache-revalidation need)

`read_if_modified` is **re-keyed from the writer tag to the object revision**:

```
read_if_modified(path, expected: &Revision) -> Result<ReadReply, BackendError>
```

It returns the full object when the stored revision differs from `expected`, and
...
(revision instead of writer tag) changes.
...
- **GCS** — `GET` with `ifGenerationNotMatch=<generation>`; a `304` is reported as
  `Precondition`.
- **Memory** — compare the stored revision to `expected`.
...
*revision-conditional* one takes its place.
...
Writes return the new `Revision`; `read` / `read_if_modified` return contents plus
`Revision`. Nothing in the trait carries metadata any more.
...
- **S3** — drop the body nonce (a content write always changes the body, so the
  ...
  `If-None-Match`.
- **GCS** — drop the metadata PATCH and the custom-metadata tags. The revision
...
- **Memory** — drop tag storage and the tag-conditional operations; compare
  revisions for `read_if_modified`.
...
3. **Keep `ObjectCache` / `ValueCache` / `Locker`** and adapt them to the slimmed
   ...
   from the writer-tag `read_if_modified` to the **revision-conditional** one, and
...
- The revision-conditional `read_if_modified` lets the cache revalidate tagless
...
- The `ObjectCache` / `Locker` caching layer is **preserved and re-pointed**,
  ...
  swapped from the writer tag to the object revision.
- S3 sheds the nonce and the `delete_if` TOCTOU window; GCS's revision token
