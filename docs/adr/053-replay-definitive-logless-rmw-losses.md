# ADR-053: Replay definitive direct-commit RMW losses
...
ADR-027's parallel locked commit).
...
This supersedes
[ADR-027](027-single-rw-parallel-lock-publish.md)'s parallel locked
single-read-write commit. Transactions that genuinely require the locked commit use it from
...
same-key mutation into the same in-doubt write would erase the earlier
transaction's recovery evidence.

The current policy sends another same-key member of that round to the locked
commit. Under sustained contention, this is not a neutral fallback: it
publishes a holder that makes subsequent direct commits ineligible. A single
local scheduling loss can therefore move an otherwise direct workload into a
...
rejections. The dominant trigger was local same-key exclusion, followed by many
direct-commit rejections against the holder created by the fallback. Replaying
the excluded transaction body instead kept the workload on direct commit and materially
improved both throughput and latency.

ADR-027 adds a third commit protocol between direct commit and the regular
locked commit. It overlaps publication of a committed transaction record with
...
to the regular locked commit. When every small-value direct commit landed directly, neither
latency nor throughput changed consistently. For 4 KiB values rejected by the
inline value limit, the regular locked commit used one more backend operation per serial
...
fallback transactions, not a claim that the two locked commits perform equally.

This exposes a distinction hidden by the existing fallback result. Some direct
commits are known not to have staged any state and only need to reevaluate
their read against a newer writer. Others are ineligible because coordination
or durable state requires the locked commit. An unavailable write whose
...
Direct commit distinguishes four semantic results:

- the direct commit landed;
- no state was staged and the transaction body should be replayed;
- the transaction requires the regular locked commit; or
- the outcome is in doubt.

These are protocol categories, not prescribed public types or names.

Body replay applies only to read-modify-write transactions for which the engine can
certify that this transaction staged no durable state. It covers a same-key
member excluded from a coordinator round and a direct commit whose observed
writer is definitively superseded before publication.

Replay uses the existing transaction replay contract. It replays the body
against current state while retaining the same transaction ID. The ID remains
unengaged: replay does not create a transaction record, acquire a lock, or
...
read-dependent computation to replay and proceeds to the regular locked commit when
it does not commit directly.

### Use one locked fallback for genuine ineligibility

The regular locked commit is the sole fallback when the direct commit
...
Do not try ADR-027's parallel locked commit before the regular locked commit. The fallback
...
This boundary is deliberately narrow. Treating every failed direct commit as
...
A same-key request excluded before staging does not inherit in-doubt state from a
...
- Contended eligible read-modify-write transactions can remain on the one-CAS,
  direct commit instead of creating a holder merely because they shared a local
  coordinator round.
- Transaction bodies may replay more often. This is already permitted by the
  transaction API's replay contract, and the measured trade-off is substantially
  less backend work and lower tail latency.
- There are two read-write commit protocols rather than three: direct commit
  and the regular locked commit. The parallel committed-record/lock race, its orphaned
  committed records, and its special recovery and cancellation cases disappear.
...
- ADR-051's direct-commit in-doubt behavior and user-visible error contract do
  not change. ADR-027's separate in-doubt lock-install outcome disappears.
...
### Send every non-landing direct commit to the locked commit
...
### Replay every failed direct commit

This avoids the locked phase transition but conflates a certified direct loss
with genuine ineligibility. A transaction could replay forever against a live
...
### Retain ADR-027 as the direct commit's first fallback
...
commit protocol, including a parallel committed-record/lock race and recovery
states that the regular locked commit does not need. Direct commit is the
optimized common case; accepting regular locked commit performance for the remainder
...
### Add a replay limit or contention backoff immediately

A limit eventually recreates the locked-commit phase transition, while
...
excessive replay amplification.
