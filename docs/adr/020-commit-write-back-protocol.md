...
inline-eligible value with one authoritative leaf CAS and no transaction record. The locked commit and write-back protocol remains the
fallback.
...
collection root (membership coordination), and the unified transaction record
...
2. For each of _its_ keys in that shard:
   ...
   - **Stage the lock** on the entry: read keys add the txid to `locked_by`
     ...
     txid as sole holder.
3. **CAS** the shard (`write_if` on its revision) with all staged changes for the
...
Reads are validated against the _same shard revision_ the CAS commits, so within a
...
transaction record (this is v1's `validate_locked_read`, relocated):

- `L` **committed** → `L` is the effective current writer; its value (from `L`'s
  transaction record) is current. A read that read the _old_ `current_writer` is stale →
...
record to `committed`, embedding the full value map (ADR-019). This single CAS is
the commit point and linearization point. If it fails because the record is
...
locks released, the transaction record is unreferenced and scheduled for GC
(ADR-022).
...
on the single committed record, not on write-back**:
...
- **Create `K`**: lock `shard(K)` (`create`) + root membership (`write`),
  ...
  which bumps the root directory generation and invalidates concurrent listers).
...
- **List**: optimistic on the root directory generation with a read-lock fallback (ADR-018);
...
DST-friendly engine while the new shard/transaction-record layout was validated end
to end. Its costs are real: on a conflict the transaction body is replayed and
every shard is re-CAS'd, and each `TxId::renew` orphans an aborted transaction
record (GC debt) until ADR-022 lands.
...
design body above: the pending record is **not** prepared up front (phase 2) but
...
- **Pending-record create**, **shard lock CAS**, and **write-back CAS** are all
  recoverable in place by read-back: the pending record's existence, the
...
- **Commit CAS**: idempotent via the txid-keyed transaction record (ADR-019); a re-issue
...
- **Read-only**: validate each read by re-checking its entry's `current_writer`
  against the read writer (no locks, no transaction record); all-match ⇒ commit.
  ...
  read still being current at validation; listing read-only uses the root
  directory-generation OCC of ADR-018.
- **Single read-write** (_superseded by
  [ADR-027](027-single-rw-parallel-lock-publish.md)_): write the committed
  transaction record (`write_if_not_exists`), then CAS the key's shard to set
  ...
  effective commit point; if it never lands, the orphan transaction record is
...
- ADR-017–020 together are behavior-complete: a transaction can validate, lock,
  ...
  transaction records — no tags, no per-key value objects, no metadata mutation.
...
- The protocol leans on leases to reclaim locks of crashed transactions
  (ADR-021) and on mark-sweep to reclaim committed records once unreferenced
