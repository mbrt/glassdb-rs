...
locked commits and leaves [ADR-022](022-garbage-collection-mark-sweep.md)'s
reclamation policy unchanged.

## Context

ADR-009 assumed that the owner could always resolve an in-doubt final-record
...
When a final-record write may have landed, the owner immediately reads the durable
...
A `pending` read proves that write attempt did not land because final records are
immutable. The in-doubt state is resolved, and a subsequent write receives a new
...
- After a definitive rejection, the owner's write did not land. Only a peer's
...
- After a write that may have landed, the missing decision is irreducibly
  in doubt, so the result is `Error::InDoubt`.
...
- The locked commit path can surface `Error::InDoubt` after a sustained failure
