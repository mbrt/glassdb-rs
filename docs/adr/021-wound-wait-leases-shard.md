...
does), and its first write *creates* the pending record with create-if-absent
semantics so a wound can never be resurrected. ADR-024 also **refined the expiry
predicate below**: the observer-relative grace (a missing transaction record appearing, or
...
that references a *missing* transaction record continues to lean on GC (ADR-022), the only
thing that can delete a still-referenced transaction record.
...
- The **lease** is the pending transaction record's timestamp, refreshed every
...
- A peer reclaims by forcing the holder's record to `aborted` with a CAS
  (`wound_tx` / `try_abort_remote_tx`); a holder referenced by a lock but with no
  record yet gets a grace period then the same treatment (`handle_unknown_tx`).
...
status live in the transaction record. The wound-wait _rule_ is unchanged; only
...
- **Lease** — a mutable "last refreshed at" time on the transaction record,
  bumped while the transaction is alive.

### The lease lives in the transaction record (no new field)

The lease is carried by the transaction record's existing `timestamp` (ADR-019),
resolving its placeholder: while the record is **pending**, `timestamp` is the
last-refresh time; once **committed**, it is the commit time and never expires.
A pending record is **expired** when, on the observer's clock,
...
its pending record's `timestamp` every `PENDING_TX_TIMEOUT / 2`, over the record's
revision, until it commits or aborts. The pending record is small (no values), so a
refresh is cheap. Its **first** write *creates* the record with create-if-absent
semantics, so an older peer that already wounded this transaction (wrote an
`aborted` record before it materialized its own pending one) wins; if a refresh
CAS finds the record already `aborted`, the transaction was wounded: the
refresher stops and the owner's commit will fail. This is `refresh_pending`,
relocated to the transaction record.
...
> aborted and released. So the window in which a *pending* record held locks was
...
> committed (the point after which write-back runs), the record never expires.
...
held by `L`, it resolves `L`'s transaction record and acts:
...
- `L` **pending** → wound-wait **combined with the lease**: `T` wounds `L` and
  ...
  forever.
- `L` referenced but its **transaction record is missing** (GC'd, or an in-doubt pending
...
Wounding `L` is a CAS of `L`'s record `pending → aborted`; a CAS that instead
...
### Wounds target only pending records
...
forcing a not-yet-final record to `aborted` is idempotent and convergent, so an
`Unavailable` outcome is resolved by re-reading the status and retrying over the
refreshed revision (the `try_abort_remote_tx` loop). A pre-commit reclaim is always
...
- **No new proto field**: the transaction record's `timestamp` doubles as the
...
- Reclamation is **lazy**: an abandoned, uncontended lock lingers until GC. This
  ...
  _non-expired_ pending record as live (reachable) so it never deletes the lease
  of a transaction that is merely slow.
- The unit of wounding stays the **whole transaction**: aborting `L` invalidates
  all of `L`'s entries at once (one record CAS), and contenders clear individual
