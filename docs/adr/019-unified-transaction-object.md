# ADR-019: Values in the unified transaction record

## Status

Accepted — implemented (`glassdb-storage::txrecord` codec + the transaction
engine)
...
with the writer's id (`glassdb-storage/src/locker.rs`). The transaction record
...
`committed_value` from that record (`Algo::validate_locked_read`).

Two things force a redesign. First, ADR-016 removes per-key value objects
entirely, so the transaction record becomes the *only* home for values. Second,
it removes tags, so status can no longer live in metadata. The natural question
is whether to keep status and values together or split the record into a small
status header plus a separate value blob.

This ADR settles the record model and lifecycle. The validate/lock/write-back
...
The transaction record at `_t/<txid>` is **unified**: small while pending, fat
once committed (it then carries the transaction's value map). We rejected
splitting it into a tiny status record plus a separate value blob:
...
- **The hot consumer needs both.** Resolving a key reads the writer's *status*
  ...
  serves both from one (cacheable) record.
- **Committed records are immutable**, so status and values cache together as one
  entry; a split adds a second object and a second cache entry for no gain.
- **The flip rewrites the record anyway.** Object stores have no partial update,
  so moving pending → committed re-PUTs the whole record regardless; splitting
...
- **Status-only consumers don't read the fat body.** Wound-wait orders by the
  ...
  small *pending* record for liveness/lease; GC decides reachability from shard
...
- **Pending** (created with `write_if_not_exists` at first lock acquisition, so
  ...
  is it locking?).
- **Committed**: a single CAS replaces the pending record with status
...
[ADR-009](009-in-doubt-conditional-writes.md)'s reasoning carries over: the record
...
idempotent — it either lands or finds the record already committed by the same
...
- A value is uploaded **exactly once**, into its transaction record; ADR-016's
  worst S3 cost — rewriting whole values to flip lock bits — is gone, and reads
  materialize from immutable, indefinitely-cacheable records.
- **Read amplification for multi-key writes**: reading one key downloads the
  ...
  to ADR-022.
- **Fat-blob liveness**: a committed record stays live while any shard references
...
- A status check is now a body GET rather than a cheap tag/metadata read. This is
  ...
  *pending* record (the one peers poll for liveness) is small.
