...
transaction now either commits direct in one leaf CAS
([ADR-051](051-inline-latest-values.md)) or uses the regular locked commit
protocol of [ADR-020](020-commit-write-back-protocol.md). This parallel locked
...
1. **Pre-check**: load the shard and resolve the entry's holders. A
   ...
   read) an effective writer that no longer matches the read writer makes the
...
2. **Issue in parallel**:
   ...
   - **W2** — one shard CAS that installs `lock_type = Write`,
     ...
     conditional on the loaded shard revision and the entry still being eligible.
...
A transaction is committed iff **both** hold: its committed record exists (W1),
and it is observable on the shard (W2 landed, or a later writer
help-forwarded it so `current_writer == txid` is observable). The record side is
unambiguous — keyed by the txid, written `committed` create-if-absent
(idempotent). The shard side is a lock, resolved exactly like the locked commit's: an
...
key may wound it, and it renews (priority preserved, `TxId::renew`) and replays the body,
exactly as the locked commit does. It could not be wounded before only because it
held nothing.
...
help-forwarded us into the chain (committed), or it never landed and the transaction record
...
- Resolving holders keeps concurrent single-key writers on the lock-free fast
  path across the write-back window instead of diverting them to the locked commit;
...
- The fast path now holds a lock, so it participates in wound-wait and lease/GC
  bookkeeping like the locked commit (its committed record lists the lock). A crash
