...
holder-free tombstones of their source, underfull leaves become merge
...
| 2    | Gate the source, quiesce, compact            | Gate L politely, compact                                          |
| 3    | Ready: source revision, split key            | Ready: L's revision, R's token, boundary `b` (L's high key)       |
| 4    | Create the sibling                           | Absorb: one CAS on R adds L's entries and the merge reservation   |
...
The absorb stops the merge and releases L's gate if R is drained, or if R has a
live structural gate, a merge reservation, or a drop intent. It also
stops the merge if the merged content does not fit in the hard cap. It removes a
structural gate of R whose holder has a final status, as any gate acquisition
does. If only R's data changed, the worker retries the absorb against the new
state of R.

The drain is one CAS on L that expects the revision recorded at Ready. If it
fails, L changed, and the drain can never land. The worker then removes the
entries below `b` and the merge reservation from R in one CAS, and deletes the
intent.

### Drained nodes

A drained node keeps its level, has an empty body, stores an explicit drained
marker, and keeps its right link to R. It covers no key, so routing and scans go
...
merge only decreases the low bound of its target, and a split only decreases the
high key of its source. Parent reconciliation and a root split create a route to
N only for keys at or above the low bound of N at that time. So the property
stays true.

Before the drain, no route reaches R for a key below `b`. R's copies of L's
entries cannot be read, and the gated L is the authority for its range. The
...
1. If L is drained and its right link is R, the merge landed. Recovery removes
   ...
   the intent.
2. If not, and R holds this merge reservation, the drain can no longer land.
   Recovery removes the entries below `b` and the reservation from R in one CAS,
   and deletes the intent.
3. In all other cases, recovery deletes the intent.
...
node can lose its low bound to a merge, so reachability alone is no longer
monotonic.

### Reads past drained nodes

Transactions need no change. Point-read validation resolves the key again,
scan validation sees the changed coverage and uses its logical fallback, and a
direct commit validates its reads in its own CAS.

A stale read, or another read that does not validate later, must read the node
after a drained node at a currentness barrier allocated after it observed the
drained node. A cached state of R from before the absorb is a real state of R,
but it does not include L's range. Splits do not have this problem: a cached
sibling always holds a committed state of its range.
...
limit, the soft byte limit, the index child limit, and the aggregate inline
budget. The absorb checks the hard cap authoritatively. With these vetoes, a new
...
Nodes get a drained marker and a merge reservation, and structural intents get a
merge variant. An older client would read a drained node as an empty live node,
and would not advance the generation at gate installation. The database
...
- Leaves that hold only cold tombstones get compacted when a neighbor admits the
  merge. Leaves without such a neighbor keep their tombstones.
...
- A stale read that passes a drained node needs one extra read at a currentness
  barrier.
...
- Splits, merges, and their recovery share one parent maintenance step. A
  ...
  path.
- Merges skip leaves that have pending holders, so busy leaves merge rarely.
...
An earlier prototype differs in these points:

- The right leaf stays as a redirect to the left leaf. Routing and scans only
  move right, so they need new rules to move left.
- The left leaf gets a gate that survives its owner's final status. The gate
  blocks writes to the left leaf until the redirect is installed, or until
  recovery runs if the worker stops.
- Only leaves merge, and parent contraction is deferred. Index levels never
  shrink, and parents keep entries for merged leaves.
- Only direct commits that miss because their keys are in two leaves trigger a
  merge. Underfull leaves without that demand never merge, so deletes do not
  shrink the topology. Here, that signal can become a later cause of the
  maintenance policy.
