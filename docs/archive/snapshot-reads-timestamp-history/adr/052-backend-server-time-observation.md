...
that party keeps a clock. S3 and Cloud Storage both report a server time on every
response.
...
across all clients of one database.

A backend declares the semantics of that observation, not only its value,
because the two available sources differ in strength:

- **Apply-anchored.** The reported time is known to be at or after the moment
  the operation applied. An object's server-assigned modification time,
  returned on the mutation itself, has this property by construction.
- **Message-anchored.** The reported time is only known to fall inside the
  request's observed duration. An HTTP `Date` response header has this
  property: nothing states whether it is stamped before or after the mutation
  became durable.

A message-anchored observation is still usable, because the stamp and the apply
both fall within one request, so they differ by at most that request's
duration, which the client's own request timeout bounds. Readers absorb that
bound in their margin. No provider guarantee is required for it.

A backend also declares the granularity of its reported time, and must not
report a time it cannot attribute to the origin. A response served or rewritten
by any cache or proxy carries an unrelated clock, so an observation is usable
only when the backend can establish that it came from the origin itself.

A backend that has no server time declares that instead; a database opened on
such a backend has no snapshot capability unless the deployment explicitly
configures it to trust client clocks, which reinstates client skew as a safety
input.
...
- The safety of a snapshot cut no longer depends on skew between client clocks.
  It depends on skew within the backend's fleet, the granularity of its reported
  time, and, for message-anchored backends, the request timeout. The snapshot
  policy's margin absorbs all three as separately named terms.
- Anchoring differs by backend, so the margin does too. Cloud Storage returns a
  server-assigned modification time on the mutation itself and is
  apply-anchored. S3 returns no modification time on a write, so it is
  message-anchored and pays the request-timeout term.
...
- Clients gain a drift detector in both directions at no cost, replacing the
  ...
  Its allowance must exceed the excursion of a leap smear, since providers smear
  while a client's own clock may not.
- No provider documents the accuracy of its reported time or publishes a bound
  on skew within its fleet, so this remains an environmental assumption of the
  same kind as trusting the backend's conditional writes. The evidence behind it
  is recorded in the design's
  [backend time assumptions](../designs/snapshot-reads.md#assumptions-about-backend-time).
...
### Require an apply-anchored time from every backend

Insisting that the reported time always be at or after the apply would remove
the request-timeout term and make one rule serve every backend. S3 returns no
modification time on a write, so meeting it would cost an extra round trip per
mutation to read one back. Declaring the anchoring instead keeps the stronger
guarantee where a backend already offers it and prices the weaker one where it
does not.
