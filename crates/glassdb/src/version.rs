...
pub(crate) async fn check_or_create_db_meta(
    ...
    match check_db_version(ctx, b, name).await {
        Ok(()) => return Ok(()),
        Err(e) if !e.is_not_found() => return Err(e),
        Err(_) => {}
    }
    match set_db_metadata(ctx, b, name).await {
        Ok(()) => Ok(()),
        Err(e) if e.is_precondition() => {
            // We raced against another instance; re-check the metadata.
            check_db_version(ctx, b, name).await
        }
        Err(e) => Err(Error::Other(format!("creating db metadata: {e}"))),
    }
}

async fn check_db_version(ctx: &Ctx, b: &Arc<dyn Backend>, name: &str) -> Result<(), Error> {
    let p = gopath::join(&[name, DB_META_PATH]);
    let meta = b.get_metadata(ctx, &p).await?;
    let got = meta
        .tags
        .get(DB_VERSION_TAG)
        .map(String::as_str)
...
async fn set_db_metadata(ctx: &Ctx, b: &Arc<dyn Backend>, name: &str) -> Result<(), Error> {
    let p = gopath::join(&[name, DB_META_PATH]);
    let mut tags = Tags::new();
    tags.insert(DB_VERSION_TAG.to_string(), DB_VERSION.to_string());
    b.write_if_not_exists(ctx, &p, Vec::new(), tags).await?;
    Ok(())
}
