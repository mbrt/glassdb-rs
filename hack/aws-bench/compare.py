...
def read_json(input_dir: Path, name: str) -> Any | None:
    """Load a result JSON when present."""
    path = input_dir / name
    if path.exists():
...
def read_first_json(input_dir: Path, *names: str) -> Any | None:
    """Load the first present JSON name, preferring the current artifact."""
    for name in names:
        result = read_json(input_dir, name)
...
def normalize_rtbench_time(
    ...
    Later rtbench versions reported simulated time after undoing
    ``--delay-scale``. Older binaries reported compressed wall time. ``factor``
    puts the latter into the current reporting domain; perfbench always uses 1.
    """
...
def with_run_identity(df: pd.DataFrame, legacy_identity: list[str]) -> pd.DataFrame:
    ...
    Current output carries `run`. For a legacy aggregate, repeated rows
...
def contention_table(a: pd.DataFrame, b: pd.DataFrame):
    def agg(df: pd.DataFrame) -> pd.DataFrame:
        df = with_run_identity(df, [])
...
def contention_stats_table(a: pd.DataFrame, b: pd.DataFrame):
    def select(df: pd.DataFrame) -> pd.DataFrame:
...
def mixed_shape_table(a: Any, b: Any) -> pd.DataFrame:
    """Per workload cell and shape throughput and latency ratios.

    Legacy per-shape operation attribution is retained when present. Current
    affinity cells deliberately mix every shape on every Database and therefore
    report operation counts only for the whole cell.
    """
    ca, cb = _indexed_mixed_cells(a), _indexed_mixed_cells(b)
    rows = []
    for key in sorted(set(ca) & set(cb)):
        run, mode, dimension, value = key
            ...
            rows.append(
                {
                    "run": run,
                    "mode": mode,
                    "layout": f"{value}%" if dimension == "affinity" else value,
                    "shape": shape,
                    "committed_a": x.get("committed", float("nan")),
                    "committed_b": y.get("committed", float("nan")),
                    # Mixed sequential sampling: True once the shape's
...
def _tidy_contention(a, b, la, lb):
    frames = []
    for src, df in ((la, a), (lb, b)):
        d = with_run_identity(df, [])
        d = d[d["overlap-pct"] == 100].copy()
...
def main() -> int:
    ...
    if a_ip is None:
        a_ip = perfbench_inline_frame(args.a)
    if b_ip is None:
        b_ip = perfbench_inline_frame(args.b)
    ...
    a_contention = perfbench_contention_frames(args.a)
    b_contention = perfbench_contention_frames(args.b)
    a_contention_samples = read_csv(args.a, "deadlock.csv")
    b_contention_samples = read_csv(args.b, "deadlock.csv")
    if a_contention_samples is None and a_contention is not None:
        a_contention_samples = a_contention[0]
    if b_contention_samples is None and b_contention is not None:
        b_contention_samples = b_contention[0]
    a_dl = normalize_rtbench_time(
        a_contention_samples,
        "deadlock.csv",
        args.rtbench_time_factor_a,
    )
    b_dl = normalize_rtbench_time(
        b_contention_samples,
        "deadlock.csv",
        args.rtbench_time_factor_b,
    )
    if a_dl is not None and b_dl is not None:
        tbl = contention_table(a_dl, b_dl)
        print_table(f"Contention latency at 100% overlap (ms, {lb}/{la})", tbl)
        summaries.append(
            summarize(
                "contention-p50",
                tbl["p50-ratio"],
    ...
    a_contention_stats = read_csv(args.a, "deadlock-stats.csv")
    b_contention_stats = read_csv(args.b, "deadlock-stats.csv")
    if a_contention_stats is None and a_contention is not None:
        a_contention_stats = a_contention[1]
    if b_contention_stats is None and b_contention is not None:
        b_contention_stats = b_contention[1]
    ...
    if a_ds is not None and b_ds is not None:
        tbl = contention_stats_table(a_ds, b_ds)
        cols = [
            "run",
            "num-keys",
            "tx-per-sec_a",
        ...
        print_table(
            f"Contention completion and protocol outcomes ({lb}/{la})", tbl[cols]
        )
        for metric, lower_is_better in [
            ("tx-per-sec", False),
            ("retries-per-tx", True),
    ...
    if a_mx is not None and b_mx is not None:
        tbl = mixed_shape_table(a_mx, b_mx)
        if not tbl.empty:
            cols = [
                "run",
                "mode",
        ...
        if not agg.empty:
            print_table(f"Mixed workload aggregate ops/tx ({lb}/{la})", agg)
            for (mode, layout), grp in agg.groupby(["mode", "layout"]):
                summaries.append(
                    summarize(
                        f"mix-agg-ops/tx[{mode}/{layout}]",
                        grp["ops-ratio"],
                        lower_is_better=True,
                    )
                )
    ...
    print(
        ...
        "variable, [unconverged] = mixed workload hit its time cap before reaching "
